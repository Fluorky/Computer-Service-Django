...
class ModelTests(TestCase):
    def setUp(self):
        ...
        self.customer = Customer.objects.create(name='John', surname='Doe', email='john@example.com',
                                                phone_number='1234567890')
        self.technician = ServiceTechnician.objects.create(name='Tech', surname='Guy', email='tech@example.com',
                                                           phone_number='9876543210', specialization='Computer Repair')
        self.service_request = ServiceRequest.objects.create(name='Service', description='Fix my computer',
                                                             requested_by=self.customer, owned_by=self.technician)
        self.part = Part.objects.create(name='Hard Drive', description='1TB HDD', quantity_in_stock=10)
        self.invoice = Invoice.objects.create(name='Invoice 21321', description='Computer repair services',
                                              requested_by=self.customer, owned_by=self.technician, total_amount=100.00)
...
class ViewTests(TestCase):
    ...
    def setUp(self):
        ...
        self.customer = Customer.objects.create(name='John', surname='Doe', email='john@example.com',
                                                phone_number='1234567890')
        self.technician = ServiceTechnician.objects.create(name='TechIT', surname='Guy', email='tech@example.com',
                                                           phone_number='9876543210', specialization='Computer Repair')
        self.service_request = ServiceRequest.objects.create(name='Service', description='Fix my computer',
                                                             requested_by=self.customer, owned_by=self.technician)
        self.part = Part.objects.create(name='Hard Drive', description='1TB HDD', quantity_in_stock=10)
        self.invoice = Invoice.objects.create(name='Invoice', description='Computer repair services',
                                              requested_by=self.customer, owned_by=self.technician, total_amount=100.00)
...
class FormTests(TestCase):
    ...
    def test_service_technician_form_valid(self):
        form_data = {'name': 'Tech', 'surname': 'Guy', 'email': 'tech@example.com', 'phone_number': '9876543210',
                     'specialization': 'Computer Repair'}
        form = ServiceTechnicianForm(data=form_data)
        self.assertTrue(form.is_valid())

    def test_customer_form_valid(self):
        form_data = {'name': 'John', 'surname': 'Doe', 'email': 'john@example.com', 'phone_number': '1234567890',
                     'address_line1': '123 Main St', 'city': 'City', 'country': 'Country'}
    ...
    def test_invoice_form_valid(self):
        form_data = {'name': 'Invoice 1', 'price': 21, 'description': 'Computer repair services', 'requested_by': 1,
                     'owned_by': 1, 'total_amount': 100.00, "payment_status": False}
